...
import java.util.*;
/**
 * Program that takes user inputs for n numbers(n>=2 in order to find integer pairs) of non duplicate integers,
 * Then find pairs of integers from the array that make the sum equal to K and display the solution. 
 * It includes the following main functions: 
 * 1)get a valid array length from user
 * 2)get valid numbers(integer, non-duplicate) from user
 * 3)get valid target (integer) from user
 * 4)find integer pairs that sum up to the target
 * @author Yanyan Yang
...
public class SumPair {
	/*
	 * This function is used to ask user to enter the length of the array
	 * It will keep asking user to enter an valid integer(>=2) until get one. 
	...
	public static int getArrayLength(Scanner s) {
		...
		while(arrayLength<2) {
			System.out.println("Please enter a integer number greater than 2: ");
			try {
	...
	/*
	 * This function is used to ask user for a target number
	 * It keeps asking for a valid input(integer number) until get it
	 * @param s: A scanner to catch the input
	 * @return integer
	 */
	...
	/*
	 * This function is used to find integer pairs that can add up to the given target
	 * It prints out all the solution pairs if there are any
	 * It prints error message when no solution is found
